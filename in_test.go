...
func Test_InInt(t *testing.T) {
	...
	for _, test := range testCases {
		if output := InInt(test.seq, test.key); output != test.expected {
			t.Errorf("Test Failed: %v sequence and %v key inputted, %v expected, recieved: %v", test.seq, test.key, test.expected, output)
		}
	}
}
...
func Test_InFloat64(t *testing.T) {
	...
	for _, test := range testCases {
		if output := InFloat64(seq, test.key); output != test.expected {
			t.Errorf("Test Failed: %v sequence and %v key inputted, %v expected, recieved: %v", seq, test.key, test.expected, output)
		}
	}
}
...
func Test_In(t *testing.T) {
	...
	if test1 != true {
		t.Errorf("Test Failed: %v sequence and %v key inputted, %v expected, recieved: %v", ar1, "apple", true, test1)
	}

	ar2 := []int{500, 100, 10, 20, 80}
	test2 := In(ar2, 800, "int")
	if test2 != false {
		t.Errorf("Test Failed: %v sequence and %v key inputted, %v expected, recieved: %v", ar2, 800, false, test2)
	}

	ar3 := []int32{500, 100, 10, 20, 80}
	test3 := In(ar3, int32(100), "int32")
	if test3 != true {
		t.Errorf("Test Failed: %v sequence and %v key inputted, %v expected, recieved: %v", ar3, 100, true, test3)
	}

	ar4 := []int64{500, 100, 10, 20, 80}
	test4 := In(ar4, int64(20), "int64")
	if test4 != true {
		t.Errorf("Test Failed: %v sequence and %v key inputted, %v expected, recieved: %v", ar4, 20, true, test4)
	}

	ar5 := []float64{50.10, 2.100, 1.570, 23.20, 80.0}
	test5 := In(ar5, 1.5, "float64")
	if test5 != false {
		t.Errorf("Test Failed: %v sequence and %v key inputted, %v expected, recieved: %v", ar5, 1.5, false, test5)
	}
